...
def critical_visibility(rho, meas, xstar, ystar):
    ...
    cp.Problem(cp.Maximize(v), ct).solve(solver=cp.MOSEK,verbose=True)
    return v.value

# Test it with X-Z measurements and 3-qubit state ensemble
...
critical_visibility(rho, meas, 0, 0)
