...
def PAM_photon_number_QRNG_MoMPy(nA,nB,nX,nY,nK,omega,W_obs):
    ...
    ct += [ sum([ G_var_vec[l][fmap(map_table,[w_P[k]])] for l in range(nL) ]) == 1.0 for k in range(nK) ]
    
    ct += [ sum([ G_var_vec[l][fmap(map_table,[w_R[x],w_P[k]])] for l in range(nL) ]) == 1.0 - omega[k][x] for k in range(nK) for x in range(nX) ]
...
for i in range(N):   
    ...
    out = PAM_photon_number_QRNG_MoMPy(nA,nB,nX,nY,nK,omega,W_obs)
